...
def write_output(text, tooltip, player):
    if player.props.status == "" or player.props.status != "Stopped":
        logger.info("Writing output")

        output = {
            "text": text,
            "class": "custom-" + player.props.player_name,
            "alt": player.props.status.lower(),
            "tooltip": tooltip,
        }
...
def on_metadata(player, metadata, manager):
    ...
    if player.get_album() != "":
        album_info = "Album: {album}".format(album=player.get_album())

    write_output(track_info, album_info, player)
